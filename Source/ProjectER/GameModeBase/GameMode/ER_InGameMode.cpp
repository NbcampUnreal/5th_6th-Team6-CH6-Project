...
void AER_InGameMode::NotifyPlayerDied(ACharacter* VictimCharacter, AController* KillerController)
{
	...
	AER_PlayerState* KillerPS = KillerController->GetPlayerState<AER_PlayerState>();
	AER_GameState* ERGS = GetGameState<AER_GameState>();

	if (!ERPS || !ERGS || !KillerPS)
		return;

	if (UER_RespawnSubsystem* RespawnSS = GetWorld()->GetSubsystem<UER_RespawnSubsystem>() )
	{
		RespawnSS->HandlePlayerDeath(*ERPS, *ERGS, *KillerPS);
